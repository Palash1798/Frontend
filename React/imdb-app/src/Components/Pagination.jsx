...
const Pagination = ({ loadMovieByPage }) => {
    ...
    const setVisiblePages = (pageNo = 1) => {
        if (pageNo === pages[0] || pageNo === pages[pages.length - 1] || !pages.length) {
            ...
            if (pageNo === pages[0]) {
                startingPage = pageNo - 5 > 1 ? pageNo - 5  : 1;
            } else if (pageNo === pages[pages.length - 1]) {
                startingPage = pageNo + 5 < totalPages ? (pageNo + 5) - maxResultSize + 1 :  totalPages - maxResultSize + 1;
            } else {
                startingPage = 1;
            }
    ...
    return (
        <div className="pagination">
            <button className="page-button" onClick={handlePrev} disabled={activePage === 1}>&lt;</button>     {/* for left arrow */}
            {getButtons()}
            <button className="page-button" onClick={handleNext} disabled={activePage === totalPages}>&gt;</button>    {/* for right arrow */}
        </div>
    )
}
